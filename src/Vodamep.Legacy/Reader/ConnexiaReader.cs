...
namespace Vodamep.Legacy.Reader
{
    public class ConnexiaReader : IReader
    {
        ...
        private readonly int _verein;


        public ConnexiaReader(string connectionstring, int verein)
        {
...
INNER JOIN (
	...
	group by d0.Adressnummer) as dd on d.Adressnummer = dd.Adressnummer and d.Datum = dd.Datum";
...
where L.Datum between @from and @to";
                ...
                var verein = connection.Query(sqlVereine, new { from = new DateTime(year, 1, 1), to = new DateTime(2017, 31, 12) }).ToArray();

                throw new NotImplementedException("todo: VereinDTO");
            }
        }
    }
}
